...
typedef struct ControlCmd
{
	...
	uint8_t gear;
	uint8_t brake;
	uint8_t turnLight; // 0关灯，1左转，2右转，3危险警示信号灯
	uint8_t stopLight; // 0关灯
...
{
public:
	enum TurnType // 道路转向类型为-1左转，0直行，1右转，3危险
	{
		TurnType_Left = -1,
		TurnType_None = 0,
		TurnType_Right = 1,
	...
	{
		if(type == TurnType_Left) // 转向灯控制指令为0关灯，1左转，2右转，3危险警示信号灯
			return 1;
...
public:
	...
	bool isSpeedValid()
	{
		return speed_validity;
	}
	
	bool isSteerAngleValid()
	{
		return steer_validity;
	}

	bool isPoseValid()
	{
		if(pose.x < 100 || pose.y < 100) {pose_validity = false;}
		else {pose_validity = true;}
